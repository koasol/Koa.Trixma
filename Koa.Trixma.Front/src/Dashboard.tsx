...
import {trixma, type System} from "./api";
        ...
        if (mounted) {
          ...
          if (data) {
            data.forEach(async (system) => {
              try {
                const {data: units} = await trixma.getUnitsBySystemId(
                  String(system.id),
                );
                if (mounted && units) {
                  setUnitCounts((prev) => ({
                    ...prev,
                    [system.id]: units.length,
                  }));
                }
              } catch (err) {
                console.error(
                  `Error fetching units for system ${system.id}:`,
                  err,
                );
              }
            });
      ...
      {loading ? (
        ...
        >
          <Typography color="text.secondary">
            No systems found. Click "Add System" to create your first system.
          </Typography>
        </Paper>
      )}
